...
export class EmailComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    if (this.currentSearch) {
      this.emailService
        ...
        .subscribe(
          (res: HttpResponse<IEmail[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInEmails(): void {
    this.eventSubscriber = this.eventManager.subscribe('emailListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IEmail[] | null, headers: HttpHeaders, page: number): void {
