...
export class FileSystemComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    if (this.currentSearch) {
      this.fileSystemService
        ...
        .subscribe(
          (res: HttpResponse<IFileSystem[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInFileSystems(): void {
    this.eventSubscriber = this.eventManager.subscribe('fileSystemListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IFileSystem[] | null, headers: HttpHeaders, page: number): void {
