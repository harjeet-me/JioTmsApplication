...
/**
 * REST controller for managing {@link com.jio.tms.v1.domain.Customer}.
 */
@RestController
@RequestMapping("/api")
public class CustomerResource {
    ...
    public ResponseEntity<List<CustomerDTO>> getAllCustomers(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("accounts-is-null".equals(filter)) {
            log.debug("REST request to get all Customers where accounts is null");
            return new ResponseEntity<>(customerService.findAllWhereAccountsIsNull(),
                    HttpStatus.OK);
        }
